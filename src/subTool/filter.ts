  ...
  for (let i = 0; i < versionHistory_filePath.length; i++) {
    ...
    if (libInfo[i].version !== '0' && matchdata_path === '') { //全てのデータを対象に特定バージョンを超えたクライアントを取得
      const versionFiltered: Client_Ver[] = data.filter(item =>
        item.verList.some(ver => getMatchedClients.isVersionGreaterOrEqual(ver.L_libVersion, libInfo[i].version))
      );
      ...
      // 変更箇所: コミットデータの保存先を変更
      const commit_data: specificCommit[] = get_specificVer_commit(versionFiltered, libInfo[i].libName, libInfo[i].version);

      // 保存先: baseOutputDir/specific-update-commit/
      const commitOutDir = path.join(baseOutputDir, 'specific-update-commit');
      output_json.createOutputDirectory(commitOutDir);

      // ファイル名: ライブラリ名-バージョン-ステータス
      const commitFileName = `${libInfo[i].libName}-${libInfo[i].version}-${status}`;

      await fs.writeFile(
        output_json.getUniqueOutputPath(commitOutDir, commitFileName, 'commits'),
...
(async () => {
  ...
  await filter_isOverVersion('../changeVersion/datasets/mydata/filter/verHist_success-12-20.json', '../../datasets/mydata/filter/matchResult.json');

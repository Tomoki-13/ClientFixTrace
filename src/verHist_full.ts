...
// - CONFIG.myDataPath (任意): 特定のライブラリを調べたい場合やデバッグ時に指定するJSON
//   ※myDataPathのファイルが存在しない場合は、testResultPathから全件自動抽出(Fullモード)します
...
(async () => {
  ...
  if (CONFIG.mode === 'full' || CONFIG.mode === 'extract') {
    ...
    // mydata.json の有無で Full / Partial を自動切り替え
    if (fs.existsSync(CONFIG.myDataPath)) {
      console.log(`[Init] mydata.json found. Running in Partial/Target mode.`);
      libVersionRanges = JSON.parse(fs.readFileSync(CONFIG.myDataPath, 'utf-8')) as any[];
      console.log(`[Init] Loaded ${libVersionRanges.length} pairs.`);
    } else {
      ...
      console.log(`[Init] Extracted ${libVersionRanges.length} valid version pairs.`);
    }

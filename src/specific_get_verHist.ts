...
async function processLibraryTask(libTask: any, targetPath: string = "", mode: RunMode, state: string, dateStr: string) {
  ...
  if (mode === 'extract' || mode === 'full') {
    ...
    const data: Item[] = await LoadJson.item('../datasets/test_result.json');
    ...
    if (population > 0) {
      ...
      verHistory = await ExtractVersion.extractVersion(client_list, libName, postVersion, state);
...
(async () => {
  ...
  for (const st of CONFIG.states) {
    for (const task of libVersionRanges) {
      ...
      await processLibraryTask(task, "", CONFIG.mode, st, dateStr);
    }
  }

  // ---------------------------------------------------------
  // パターンB: 【Analyze実行】既存のデータを使って分類だけやり直す場合
  // ---------------------------------------------------------
  // const historyPath = CONFIG.analyzeTargetHistoryPath;
  // const task = libVersionRanges[0];
  // await processLibraryTask(task, historyPath, 'analyze', 'success', dateStr);
})();